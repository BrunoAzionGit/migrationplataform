...
- ~~Formato da API do Edge SQL~~ — **confirmado e em produção**: banco `azion-migration-jobs` criado, schema aplicado via `POST .../databases/<id>/query` com `{"statements": [...]}` (uma statement por item do array, sem placeholder nativo — `EdgeSqlClient` escapa os parâmetros no client). Ver `packages/core/src/db/edgeSql.ts`.
- **Object Storage — client escrito, ainda não testado ponta a ponta**: bucket `storage-azion-migration-artifacts` criado (Restricted) com credencial S3. `ObjectStorageClient` usa `@aws-sdk/client-s3` contra `https://s3.us-east-005.azionstorage.net` (região confirmada na doc, path-style), mas a conectividade real não pôde ser validada no ambiente onde o código foi escrito (rede do sandbox bloqueava o domínio) — testar `putInput`/`get` de verdade antes de confiar no fluxo completo.
  - Nota: a Azion recomenda **não** acessar o endpoint S3 diretamente para *entrega de conteúdo a usuário final em escala* (rate limit, sem cache de edge) — nesses casos o certo é Edge Application + Connector na frente do bucket. Nosso uso atual (ler/gravar input e relatório de job, backend-only, baixo volume) é o caso de uso que a própria doc descreve como pretendido para acesso via API/S3. Se um dia surgir a necessidade de um link compartilhável do relatório direto pro cliente abrir no navegador, isso passa a contar como "entrega a usuário final" e vale reavaliar para Application+Connector.
